...
contract RewardApeFactoryV2 is Ownable {
  ...
  function deployDefaultPoolContract(
        ...
        uint256 _startTime,
        uint256 _bonusEndTime) public {
    deployPoolContract(_stakeToken, _rewardToken, _rewardPerSecond, _startTime, _bonusEndTime, defaultOwner);
  }
  ...
  function deployPoolContract(
    ...
    pool.initialize(_stakeToken, _rewardToken, _rewardPerSecond, _startTime, _bonusEndTime);

    pool.transferOwnership(_owner);

    poolManager.addPool(address(pool), false);

    emit DeployedPoolContract(address(pool), address(_stakeToken), address(_rewardToken), _rewardPerSecond, _startTime, _bonusEndTime, _owner);
  }
  ...
  function deployComputedPoolContract(
    ...
    (uint256 rewardPerSecond, uint256 bonusEndTime) = calculatedConfig(_totalRewards, _startTime, _secondsDuration);
    deployPoolContract(_stakeToken, _rewardToken, rewardPerSecond, _startTime, bonusEndTime, _owner);
  }

  function calculatedConfig(
    uint256 _totalRewards,
    uint256 _startTime,
    uint256 _secondsDuration) public pure returns (uint256 rewardsPerSecond, uint256 bonusEndTime) {
