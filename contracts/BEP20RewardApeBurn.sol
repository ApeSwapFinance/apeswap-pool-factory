...
contract BEP20RewardApeBurn is Ownable, Initializable {
    ...
    // The block number when deposits are rejected.
    uint256 public lockBlock;
    // Indicates wether emegerncyWithdraw can be called.
    ...
    event EmergencyToggled(bool emergencyActive);
    ...
    /// Stake tokens into the contract to earn rewards or call with _amount 0 to just harvest.
    /// @dev Since this contract needs to be supplied with rewards we are
    ///  sending the balance of the contract if the pending rewards are higher
    /// @param _amount The amount of staking tokens to deposit
    function stake(uint256 _amount) public {
        require(emergencyActive == false, "Uh oh, looks like sumtin is going on.");
            ...
            if(pending > 0) {
                ...
                if(currentRewardBalance > 0) {
                    if(pending > currentRewardBalance) {
                        ...
                        emit Withdraw(msg.sender, currentRewardBalance);
                    } else {
    ...
    function toggleEmergency() external onlyOwner {
        ...
        emit EmergencyToggled(emergencyActive);
    }

    /// @dev burn all the tokens 
    function burnItAll() external onlyOwner {
        require(block.number >= bonusEndBlock, 'Too early to burn ser.');
        uint256 burnAmount = STAKE_TOKEN.balanceOf(address(this));
        STAKE_TOKEN.safeTransfer(burnAddress, burnAmount);
        emit Burn(burnAmount);
    }

    /* Emergency Functions */

    // Withdraw without caring about rewards. Would only activate it if something went wrong with staked tokens.
    function emergencyWithdraw() external {
        require(emergencyActive == true, "No emergency going on right now ser.");
        UserInfo storage user = userInfo[msg.sender];
